...
describe("useErrorHandler", () => {
  ...
  describe("Basic Error Handling", () => {
    it("should handle errors with default options", () => {
      ...
      expect(mockToastError).toHaveBeenCalledWith(
        "Something went wrong: Test error message"
      );
    });
  ...
  describe("Options Configuration", () => {
    ...
    it("should use custom context from options", () => {
      ...
      const { result } = renderHook(() =>
        useErrorHandler({ context: customContext })
      );
      const testError = new Error("Test error");
    ...
    it("should merge additional context with options context", () => {
      ...
      const { result } = renderHook(() =>
        useErrorHandler({ context: optionsContext })
      );
      const testError = new Error("Test error");
  ...
  describe("Async Error Handling", () => {
    ...
    it("should handle failed async operations with fallback", async () => {
      ...
      let asyncResult;
      await act(async () => {
        asyncResult = await result.current.handleAsyncError(
          asyncFn,
          "fallback"
        );
      });
...
describe("useFormErrorHandler", () => {
  ...
  it("should handle form errors with form context", () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: Form error message"
    );
  });
...
describe("useAPIErrorHandler", () => {
  ...
  it("should handle API errors with API context", () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: API error message"
    );
  });
...
describe("Error Message Handling", () => {
  it("should handle different error types", () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: Error object message"
    );
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: String error message"
    );
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: Unknown error message"
    );
  });
});
...
describe("Integration with Error Types", () => {
  it("should work with AppError types", () => {
    const { result } = renderHook(() => useErrorHandler());
    const appError = new Error("App error") as Error & {
      statusCode: number;
  ...
  it("should work with regular Error types", () => {
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      "Something went wrong: Regular error message"
    );
  });
});
