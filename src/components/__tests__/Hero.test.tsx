...
jest.mock("../animations", () => ({
  ...
  PulsingBadge: ({ children }: AnimationProps) => (
    <div data-testid="pulsing-badge">{children}</div>
  ),
}));

// Mock HeroAnimations component
jest.mock("../HeroAnimations", () => ({
  HeroAnimations: ({ primaryButton, secondaryButton }: HeroAnimationsProps) => (
    <div data-testid="hero-animations">
      <div data-testid="primary-button">{primaryButton?.text || "none"}</div>
      <div data-testid="secondary-button">
...
describe("Hero", () => {
  ...
  describe("Component Rendering", () => {
    it("should render with required props", () => {
      ...
      expect(
        screen.getByText(
          "Responsible AI solutions for healthcare and education"
        )
      ).toBeInTheDocument();
    });
    ...
    it("should render with proper content wrapper", () => {
      ...
      const contentWrapper = screen
        .getByText("Welcome to SAFE AI [4U]")
        .closest("div");
  ...
  describe("Animation Integration", () => {
    it("should render FadeInUp animations with correct delays", () => {
      ...
      // Check delays
      const badgeFadeIn = fadeInUpElements.find(
        el => el.getAttribute("data-delay") === "0.1"
      );
      const titleFadeIn = fadeInUpElements.find(
        el => el.getAttribute("data-delay") === "0.2"
      );
      const descriptionFadeIn = fadeInUpElements.find(
        el => el.getAttribute("data-delay") === "0.3"
      );
  ...
  describe("Props Validation", () => {
    ...
    it("should handle long text content", () => {
      const longTitle =
        "This is a very long title that should still be displayed correctly in the hero section";
      const longDescription =
        "This is a very long description that should still be displayed correctly in the hero section and should wrap properly on different screen sizes";
    ...
    it("should handle special characters in text", () => {
      const specialText =
        'AI & Machine Learning: "Responsible" Development (2024)';
