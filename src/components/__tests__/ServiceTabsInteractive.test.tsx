...
jest.mock("../animations/FadeInUp", () => ({
  FadeInUp: ({ children, delay }: AnimationProps) => (
...
jest.mock("../animations/StaggerContainer", () => ({
  StaggerContainer: ({ children, className }: AnimationProps) => (
    <div data-testid="stagger-container" className={className}>
      {children}
    </div>
  ),
  StaggerItem: ({ children }: AnimationProps) => (
    <div data-testid="stagger-item">{children}</div>
  ),
}));

jest.mock("../animations/InteractiveElements", () => ({
  FloatingCard: ({ children }: AnimationProps) => (
    <div data-testid="floating-card">{children}</div>
  ),
}));

jest.mock("../animations/AnimatedProgress", () => ({
  AnimatedProgress: ({ value, className, delay }: ProgressProps) => (
    <div
...
jest.mock("../animations/AnimatedCounter", () => ({
  AnimatedCounter: ({ value, suffix }: CounterProps) => (
    <span
...
describe("ServiceTabsInteractive", () => {
      ...
      cards: [
        {
          ...
          content:
            "Comprehensive AI strategy planning and implementation guidance.",
      ...
      cards: [
        {
          ...
          content:
            "Custom AI solution development for specific business needs.",
  ...
  describe("Component Rendering", () => {
    ...
    it("should render all tab content areas", () => {
      ...
      expect(
        screen.getByRole("tabpanel", { name: "AI Consulting" })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tabpanel", { name: "AI Development" })
      ).toBeInTheDocument();
      expect(
        screen.getByRole("tabpanel", { name: "AI Education" })
      ).toBeInTheDocument();
    });
  ...
  describe("Tab Navigation", () => {
    ...
    it("should set specified defaultTab as active", () => {
      render(
        <ServiceTabsInteractive tabs={mockTabs} defaultTab="development" />
      );
    ...
    it("should switch tabs when clicked", async () => {
      ...
      expect(
        screen.getByRole("tab", { name: "AI Consulting" })
  ...
  describe("Card Content Display", () => {
    ...
    it("should display card content text", () => {
      ...
      expect(
        screen.getByText(
          "Detailed analysis of AI market trends and opportunities."
        )
      ).toBeInTheDocument();
    });
  ...
  describe("Animation Components Integration", () => {
    ...
    it("should render AnimatedProgress with correct values", () => {
      ...
      const progress85 = progressBars.find(
        bar => bar.getAttribute("data-value") === "85"
      );
      const progress70 = progressBars.find(
        bar => bar.getAttribute("data-value") === "70"
      );
      const progress60 = progressBars.find(
        bar => bar.getAttribute("data-value") === "60"
      );
    ...
    it("should render AnimatedCounter with correct values and suffixes", () => {
      ...
      const counter85 = counters.find(
        counter => counter.getAttribute("data-value") === "85"
      );
      const counter70 = counters.find(
        counter => counter.getAttribute("data-value") === "70"
      );
  ...
  describe("Badge Display", () => {
    it("should render badges with correct styling", () => {
      ...
      const badges = screen.getAllByText(
        /Strategy|Planning|Implementation|Research|Analysis|Development|Custom|Solutions|Education|Training|Certification|Workshops|Hands-on/
      );
  ...
  describe("Edge Cases", () => {
    ...
    it("should handle invalid defaultTab gracefully", () => {
      render(
        <ServiceTabsInteractive tabs={mockTabs} defaultTab="invalid-tab" />
      );
