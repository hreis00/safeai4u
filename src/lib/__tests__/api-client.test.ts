...
describe("API Client", () => {
  ...
  describe("apiRequest", () => {
    ...
    it("should handle timeout", async () => {
      ...
      await expect(apiRequest("/api/test", { timeout: 50 })).rejects.toThrow(
        NetworkError
      );
    });
  ...
  describe("safeApiCall", () => {
    ...
    it("should return fallback on failed API call", async () => {
      ...
      expect(mockLogError).toHaveBeenCalledWith(expect.any(Error), undefined);
    });
    ...
    it("should pass context to logError", async () => {
      ...
      expect(mockLogError).toHaveBeenCalledWith(expect.any(Error), context);
    });
  });
